...
public class AdafruitSSD1306
{
  ...
  private final static int SPI_DEVICE = 0;

  private int width = 128, height = 32;
  private int clockHertz = 8000000;
  ...
  public AdafruitSSD1306(int w, int h)
  {
    initSSD1306(w, h);
  }
  ...
  private void initSSD1306(int w, int h)
  {
    ...
    /*  try
    {
      spiDevice = new SpiDeviceImpl(SpiChannel.CS0, clockHertz);
    }
    catch (Exception ex)
    {
      ex.printStackTrace();
  ...
  public void display()
  {
    ...
    dcOutput.high();
    // Write buffer.
    //    byte[] ba = new byte[this.buffer.length];
    //    for (int i=0; i<this.buffer.length; i++)
    //      ba[i] = (byte)this.buffer[i];
    //    try { spiDevice.write(ba, 0, ba.length); }
    //    catch (IOException ioe) { ioe.printStackTrace(); }
