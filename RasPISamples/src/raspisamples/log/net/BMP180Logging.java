...
public class BMP180Logging
{
  ...
  public final static int BMP180_ADDRESS = 0x77; // i2cdetect
  // Operating Modes
  ...
  public BMP180Logging()
  {
    this(BMP180_ADDRESS);
  }
  
  public BMP180Logging(int address)
  {
    try
    {
      // Get i2c bus
  ...
  public static void main(String[] args)
  {
    ...
    while (true)
    {
      ...
      try
      {
        String url = LOGGER_URL + "?board=OlivRPi1&sensor=BMP180&type=TEMPERATURE&data=" + NF.format(temp);
        String response = HTTPClient.getContent(url);
        JSONObject json = new JSONObject(response);
        System.out.println("Returned\n" + json.toString(2));
        try { Thread.sleep(1000); } catch (Exception ex) {} // To avoid duplicate PK
        url = LOGGER_URL + "?board=OlivRPi1&sensor=BMP180&type=PRESSURE&data=" + NF.format(press / 100);
